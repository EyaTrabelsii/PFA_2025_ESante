...
export const getUserInfo = async (userId, token) => {
  try {
    const response = await axios.get(`${USER_SERVICE_URL}/api/v1/users/profile/${userId}`, {
...
export const getAppointmentById = async (appointmentId) => {
  try {
    const response = await axios.get(`${RDV_SERVICE_URL}/api/v1/appointments/${appointmentId}`);
    return response.data.data;
  } catch (error) {
...
export const isUserOnline = async (userId) => {
  try {
    const response = await axios.get(
      `${MESSAGING_SERVICE_URL}/api/v1/messages/users/${userId}/online-status`
    );
